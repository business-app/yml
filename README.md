...
`measure` (Необязательное, Может быть `unit`, `kg`, `gramm`, `lit`, `mlit`, `sec`, `day` или `hour`). Единица измерения, в которых измеряется товар. Например, килограммы для картофеля и штуки – для пакетов молока.  
